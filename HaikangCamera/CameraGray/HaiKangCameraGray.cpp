//
// Created by yuesang on 22-12-5.
//

#include "HaiKangCameraGray.h"

HaiKangCameraGray::HaiKangCameraGray()
{
}
...
int HaiKangCameraGray::StartDevice(int serial_number)
{   //打开设备
...
//                 return -1;
//             }
//             HaiKangCameraGray::PrintDeviceInfo(pDeviceInfo);
//         }
...
bool HaiKangCameraGray::SetStreamOn()
{   //开始采集
    //设置触发模式为off
    ...
    //创建相机图像采集线程
    // pthread_t tid;
    // nRet = pthread_create(&tid, NULL, HaiKangCameraGray::WorkThread, handle);
    // if(MV_OK == nRet)
    // {
...
bool HaiKangCameraGray::SetResolution(int width, int height)
...
bool HaiKangCameraGray::SetExposureTime(float ExposureTime)
{   //设置曝光时间
...
bool HaiKangCameraGray::Set_BALANCE(int value, unsigned int value_number)
{   //手动白平衡（具有记忆功能））
    //关闭自动白平衡
...
bool HaiKangCameraGray::Set_Gamma(bool set_status,double dGammaParam)
{   //设置Gamma值
    if(set_status == true)
    {
...
bool HaiKangCameraGray::Color_Correct(bool value)
{   //设置色彩校正
    if(value == true)
    {
...
bool HaiKangCameraGray::Set_Contrast(bool set_status,int dContrastParam)
{   //设置对比度
    if(set_status == true)
    {
...
bool HaiKangCameraGray::UpdateTimestampOffset(std::chrono::_V2::steady_clock::time_point time_start)
{   //计算时间戳偏移
...
int HaiKangCameraGray::Get_TIMESTAMP()
{   //获取时间戳
...
bool HaiKangCameraGray::GetMat(cv::Mat &Src)
{
